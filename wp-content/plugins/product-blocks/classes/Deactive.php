...
class Deactive{
	...
	public function wopb_tracking_callback(){
		...
		if( sanitize_text_field($_GET['productx_tracking']) == 'yes' ) {
    	...
    	<div class="wopb-modal" id="wopb-deactive-modal">
            <div class="wopb-modal-wrap">
                ...
                <div class="wopb-modal-body">
                    <h3><?php esc_html_e( "If you have a moment, please let us know why you are deactivating ProductX:", "product-blocks" ); ?></h3>
                    <ul class="wopb-modal-input">
						<?php foreach ($this->get_settings() as $key => $setting) { ?>
							<li>
								<label>
									<input type="radio" <?php echo ($key == 0 ? 'checked="checked"' : ''); ?> id="<?php esc_attr_e($setting['id']); ?>" name="<?php esc_attr_e($this->PLUGIN_SLUG); ?>" value="<?php esc_attr_e($setting['text']); ?>">
									<div class="wopb-reason-text"><?php esc_html_e($setting['text']); ?></div>
									<?php if( isset($setting['input']) && $setting['input'] ) { ?>
										<textarea placeholder="<?php esc_attr_e($setting['placeholder']); ?>" class="wopb-reason-input <?php echo ($key == 0 ? 'wopb-active' : ''); ?> <?php esc_html_e($setting['id']); ?>"></textarea>
									<?php } ?>
								</label>
							</li>
						<?php } ?>
