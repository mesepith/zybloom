			...
			if ( $templates && $templates != 'none' ) {
				...
				if ($the_query->have_posts()) {
					...
					if (in_array('et-fb', $body_class)) {
						echo wopb_function()->set_css_style($templates, true);
					} else {
				...
				if (in_array('et-fb', $body_class)) {
					$content = '<p style="text-align:center;">'.sprintf( esc_html__( 'Pick a Template from your saved ones. Or create a template from: %s.' , 'product-blocks' ) . ' ', '<strong><i>' . esc_html( 'Dashboard > ProductX > Saved Templates', 'product-blocks' ) . '</i></strong>' ).'</p>';
				}
			}
