...
class Saved_Templates {
    ...
    public function custom_head_templates() {
        if( 'wopb_templates' == get_current_screen()->post_type && (!defined('WOPB_PRO_VER')) ) {
            ...
            if( $post_count > 0 ) { ?>
                <span class="wopb-saved-templates-action" data-link="<?php echo esc_url(wopb_function()->get_premium_link('', 'menu_saved_template_go_pro')); ?>" data-text="Go Pro for Unlimited Templates" data-count="<?php esc_attr_e($post_count); ?>" style="display:none;"></span>
            <?php }
        }
    }
