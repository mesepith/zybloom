jQuery(document).ready(function ($) {
  ...
  $('#inboxWebmail_table_lbl').on('click', "#add_lbl", function(e) {
      $('tbody').append('<tr class="add_row"><td><input type="text" class="form-control" name="lbl_name[][]" value="" required="required" /></td><td><input type="text" class="form-control" name="lbl_code[][]" value="" required="required" /></td><td class="text-center"><button type="button" class="badge badge-info delc" id="delete_lbl" title="Delete label">X</button></td><tr>');
      e.preventDefault();
  });
...
$('#email-nav').on('click', "#refresh_data", function(e) {
	...
	$('tbody').append('<tr class="add_row"><td>&nbsp;</td><td><input style="border:0px;" name="file[]" type="file" multiple /></td><td class="text-center"><button type="button" class="badge badge-info delc" id="delete_file" title="">X</button></td><tr>');
	e.preventDefault();
   });
 ...
 function inboxWebmail_deleteLabel(id) {
  ...
  jQuery.ajax({
      type: "GET",
      url: jQuery("#inlbl_del_url").val()+id,
      data: 1,
      cache: false,
      success: function(data){
      }
