...
/**
 * PPEX_WC_B2B_PG_Client
 */
class PPEX_WC_B2B_PG_Client implements PPEX_PG_Interface {
  ...
  public function init_txn($wc_order_id) {
    ...
    $payment_instrument = new PPEX_Payment_Instrument("PAY_PAGE");
  ...
  public function render_payment_ui($wc_order_id) {
    ...
    if ($pg_v1_pay_response->get_success() == false) {   
      ...
      add_action('the_content', array(&$this, 'phonepe_show_message'));
      ...
      $img_src = esc_url(WP_PLUGIN_URL . '/' . plugin_basename(dirname(__FILE__))) . '/images/loader.gif';
      $callback_url = esc_url_raw($this->get_redirect_url_for_order($pg_v1_pay_response->get_merchant_transaction_id()));
      if($this->plugin_context->get_environment() == PPEX_Constants::PRODUCTION){ ?>     
      <script src="https://mercury.phonepe.com/linchpin/checkout/1.0.4/phonepe.js"></script>
      <?php }else if($this->plugin_context->get_environment() == PPEX_Constants::UAT){  ?>
      <script src="https://mercury-stg.phonepe.com/linchpin/checkout/0.4.13/phonepe.js"></script>  
      <?php }else{ ?>
      <script src="https://mercury-stg.phonepe.com/linchpin/checkout/0.4.13/phonepe.js"></script>
      <?php } ?>
      <script>
        const callback = response => {
          if (response == "CONCLUDED") {
            ...
            window.location.replace("<?php echo $callback_url; ?>");
          }
          if (response == "USER_CANCEL") {
            ...
            window.location.replace("<?php echo esc_url(get_the_permalink($checkout_url)) . '/' . $wc_order_id . '/?phonepe_response=Your+payment+is+cancelled.&type=error'; ?>");
          }
        }
        document.querySelectorAll('li.method strong')[0].innerHTML = "<?php echo esc_html(PPEX_PG_Constants::PAYMENT_METHOD_NAME); ?>";
        window.PhonePe.transact({
          tokenUrl: "<?php echo esc_url($redirect_pay_url); ?>",
          callback
        });
      </script>
      <?php
    ...
    public function handle_callback_response($ppex_callback) {
        $merchant_transaction_id = $ppex_callback->get_merchant_transaction_id();
        ...
        $amount_returned_in_paisa = $ppex_callback->get_amount_in_paisa();
        ...
        $ppex_event->set_merchant_transaction_id($merchant_transaction_id);
        $ppex_event->set_amount_in_paisa($amount_in_paisa);
        $ppex_event->set_grouping_key($ppex_callback->get_merchant_id() . ':' . $ppex_callback->get_merchant_transaction_id());
        $ppex_event->set_code($ppex_callback->get_pay_response_code());
        $this->network_manager->post_event($ppex_event, $this->merchant_context, $this->plugin_context);
        ...
        $this->status_update_for_order($ppex_callback->get_pay_response_code(), $wc_order_id, $merchant_transaction_id);
    }
  ...
  public function check_phonepe_response($merchant_transaction_id) {
    ...
    $amount_returned = 0;
    $res_status = $this->network_manager->check_status($x_verify, $merchant_id, $merchant_transaction_id, $amount_in_paisa, $this->merchant_context, $this->plugin_context);

    if ($res_status['code'] != PPEX_Constants::TXN_NOT_FOUND) {
        $amount_returned = $res_status['data']['amount'];
    }else{
    ...
    if ($res_status['code'] == PPEX_Constants::SUCCESS && $amount_in_paisa == $amount_returned) {
        $this->status_update_for_order($res_status['code'], $wc_order_id, $merchant_transaction_id);
        $redirect_url = $order->get_checkout_order_received_url();
        ...
        if ($amount_in_paisa != $amount_returned) {
            $msg .= "Amount mismatch!"; 
        } else if ( isset($res_status['code']) && $res_status['code'] == PPEX_Constants::DECLINED ) {
  ...
  public function check_pending_status() {
    ...
    foreach ($pending_orders_query as $order) {
      ...
      $x_verify    = self::calculate_status_checksum($merchant_id, $merchant_transaction_id, $salt_key, $salt_index);
      $res_status  = $this->network_manager->pg_status_check($x_verify, $merchant_id, $merchant_transaction_id, $this->plugin_context->get_environment());

      $code = $res_status['code'];
  ...
  public function status_update_for_order($code, $wc_order_id, $merchant_transaction_id, $msg=""){
    ...
    if ($code == PPEX_Constants::SUCCESS) {
      ...
      $order->add_order_note("Your payment is successful - merchant transaction id: " . $merchant_transaction_id);
      ...
      $order->add_order_note("Payment Transaction Failed" . ' - merchant transaction id: ' . $merchant_transaction_id);
    } 
    else {
