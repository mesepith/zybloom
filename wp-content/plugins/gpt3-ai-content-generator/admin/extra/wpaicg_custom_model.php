...
<script>
    jQuery(document).ready(function ($){
        ...
        function toggleFieldsBasedOnProvider(provider) {
            const modelSelectDiv = $('.wpaicg_custom_template_model').closest('.wpaicg-form-field');
        ...
        wpaicg_template_save_post.click(function (){
            ...
            if(title === ''){
                ...
                $.ajax({
                    url: wpaicg_template_ajax_url,
                    data: {action: 'wpaicg_template_post',post_type: post_type, model: model, provider:provider, google_model: google_model, azure_deployment: azure_deployment, duration: duration, title: title, excerpt: excerpt, content: content, description: description, tokens:wpaicg_tokens, words: wpaicg_words_count,'nonce': '<?php echo wp_create_nonce('wpaicg-ajax-nonce')?>'},
                    type: 'POST',
                    dataType: 'JSON',
                    beforeSend: function () {
                    ...
                    success: function (res){
                        if(res.status === 'success'){
                            ...
                            if($('.wpaicg_custom_template_id').length && $('.wpaicg_custom_template_id') !== ''){
                                alert('<?php echo esc_html__('Template updated successfully.','gpt3-ai-content-generator')?>');
                                wpaicgRmLoading(btn);
                                window.location.reload();
                            }
                            else{
                                alert('<?php echo esc_html__('New template created successfully.','gpt3-ai-content-generator')?>');
                                window.location.reload();
                            }
                        }
                        else alert(res.msg);
